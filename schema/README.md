...
| `artifact-acceptance-report.schema.json` | Runtime copy of the unchanged M1 four-state report contract; candidate, delivery, resource and author evidence bindings are checked separately. |
...
`delivery-recipe.schema.json` freezes provider-neutral workers, source scope, context bounds and cumulative resource limits for `generation-report-1`. Quality scores are reports, not acceptance gates.
