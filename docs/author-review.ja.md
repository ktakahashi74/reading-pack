...
このページは`review export / plan / apply`のMarkdownレビューと公開判断を説明する。`artifact-acceptance-1`の`pipeline finalize --review`は別のJSON判断入力を使い、判断記録だけを追記する。承認済み配布物の構築や公開への自動接続はない。 [制作ガイド](pipeline-artifact-workflow.ja.md).
