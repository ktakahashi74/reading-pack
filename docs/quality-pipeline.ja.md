...
本文を入力して自動制作する場合は`reading-pack pipeline start`を使う。任意の著者追加資料も取り込み、出典付き生成、自動評価、上限付き修正を経て、一つの著者確認票まで進む。詳細は[自動制作パイプライン](automatic-pipeline.ja.md)を参照する。以下の個別コマンドは、手動操作と検査にも利用できる。
