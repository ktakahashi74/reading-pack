...
Reference records may omit `url` when they contain a bibliographic citation in `label` and the source supplies no web address. Omit the property in JSON; leave the existing `url` column empty in CSV. Do not substitute an invented address. Official companion declarations still require their URL, scope and retrieval policy. A citation alone does not provide access to the referenced work.
