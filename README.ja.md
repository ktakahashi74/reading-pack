...
新規制作の推奨入口は`pipeline deliver`。既定で全モジュールを現在の原稿から新規生成し、章ごとの固定回数で全生成項目を評価する。旧項目の継承には`--seed PROJECT --inherit-seed`の明示を要する。Packと定量評価報告を返す。低得点や評価未完了でも生成物を保持し、採否はユーザーに委ねる。[生成・評価納品の操作](docs/pipeline-delivery.ja.md)。

以下は既存契約の説明。

`artifact-acceptance-1`は、内容・指示・ローカル配布物の直接検査、固定資源枠、一括修正最大1回、既存候補の別再判定、候補hashに結び付いた著者確認票までを実行する。標準の設問生成・回答・採点は0回である。無指定は`legacy-reader-evaluation-1`を維持する。 [直接検査による制作](docs/pipeline-artifact-workflow.ja.md)。[モデル比較試験](docs/pipeline-model-comparison.ja.md)は任意の開発・運用試験であり、個別Packの合格条件ではない。
...
自動ワークフローは実測適格性のないalphaである。従来契約のproductionモードには、工程全体の時間・費用枠と実測済みのワークフロー証明を必要とする。従来の研究用コマンドは明示的な`--experimental`を必要とする。制御試験の成功は実書籍の完成実績ではない。計画、実測、現在の限界は[時間・費用・品質の予測可能性](docs/predictable-production.ja.md)にまとめる。

本文を入力すると、`reading-pack pipeline start`が構造抽出、内容生成、品質評価、上限付きの自動修正を実行し、合格した候補を著者確認票へまとめます。著者が用意した付録なども任意で追加できます。 参考文献にURLがなくても、本文中の書誌情報を利用できます。章が処理区間をまたぐ場合、審査に使う原文を上限付きで前後へ広げます。設定と実行方法は[自動制作ガイド](docs/automatic-pipeline.ja.md)を参照してください。

生成前に、必須情報の範囲を固定します。監査の指摘は、Pack全体と、登録済み付録を含む上限付きの原文で独立に再判定します。任意の改善案は別に保存し、修正や読解試験の停止を起こしません。確認された意味上の誤りと、根拠の未解決は引き続き合格を妨げます。
...
本文の出典検査と読解試験を通過した後、著者が内容と公開条件を確認します。上限到達や最終試験の不合格では、停止理由と未解決事項を記録します。実装更新時には、停止した下書きと固定済みの設問を保持して再評価できます。著者提供データの保護、指定記録の未承認改訂への明示許可、固定試験、再開方法は詳細ガイドで説明します。
...
依存パッケージが手元にない場合、インストール時にパッケージ索引へ接続します。インストール後の基本機能はローカルで動き、通信しません。PDFの取り込みには、ローカルにあるPopplerの`pdfinfo`と`pdftotext`も使います。縦書きPDFのレイアウト復元には`pdftohtml`も使います。
...
PDFの章節構造も最後の著者確認に含みます。自動pipelineは、縦書きPDFの文字位置・大きさから構造候補を復元し、本文に基づく選別と独立検証を行います。`pdf-vertical`にOCR機能はなく、文字層のないスキャンや対応外の組版は明示的に停止します。
