...
- Render offsets into the extracted text as chapter, section, and section start page when a Pack records section pages. A generated Pack located all of its records as `source.txt#normalized-text:a-b`; readers cannot look those up, and assistants repeated them as places in the book. Each section overview claim spans its section, so an offset range resolves to the sections it overlaps; a range covering a whole chapter or more than three sections reads as the chapter. The Pack data keeps the offsets, so source verification is unchanged, and Packs without section pages render byte for byte as before.
